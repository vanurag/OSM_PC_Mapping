...
def get_corner(idx):
	# idx = random index from point cloud

	# select 150 nearest indexes
	nearest_idx, sqr_dist = kd.nearest_k_search_for_point(p, idx, 150)

	grid_p = pcl.PointCloud()
	
	# gather points for sub point cloud	
	temp_list = list()
	...
	if(grid_p.size - len(indices) < 5):
		# only 5 indices left we have found a line and not a corner
		return None
	...
	if(g2.size):
		# find a second line in the remaining point cloud
		...
		# if angle not close to 90 degree, reject
		if angle < 0.6 or angle > (np.pi/2) - 0.6:
